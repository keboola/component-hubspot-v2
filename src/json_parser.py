...
class FlattenJsonParser:
    ...
    def __init__(
        self, child_separator: str = "_", max_parsing_depth=DEFAULT_MAX_PARSE_DEPTH
    ):
    ...
    def _flatten_row(self, nested_dict):
        ...
        def _flatten(dict_object, name_with_parent="", current_depth=0):
            if (
                isinstance(dict_object, dict)
                and current_depth <= self.max_parsing_depth
            ):
                for key in dict_object:
                    new_parent_name = self._construct_key(
                        name_with_parent, self.child_separator, key
                    )
                    new_depth = current_depth + 1
                    _flatten(
                        dict_object[key],
                        name_with_parent=new_parent_name,
