...
class Component(ComponentBase):
    ...
    def run(self):
        ...
        for endpoint_name in self._configuration.endpoints.enabled:
            if endpoint_name == "custom_object":
                custom_object_types = (
                    self._configuration.additional_properties.custom_object_types
                )
                for custom_object in custom_object_types:
                    self.get_custom_objects(custom_object)
                    self._created_tables[custom_object] = self._table_handler_cache[
                        custom_object
                    ].table_definition
            else:
                self.process_endpoint(endpoint_name)
                self._created_tables[endpoint_name] = self._table_handler_cache[
                    endpoint_name
                ].table_definition
    ...
    def _validate_custom_objects(self):
        if self._configuration.endpoints.custom_object:
            if not self._configuration.additional_properties.custom_object_types:
                raise UserException(
                    "Custom object types must be specified in the configuration."
                )
    ...
    def since_fetch_date(self) -> int:
        ...
        if self._configuration.fetch_settings.fetch_mode == FetchMode.INCREMENTAL_FETCH:
            since_fetch_date: int = int(
                self._parse_date(self._configuration.fetch_settings.date_from)
            )
    ...
    def get_email_events(self) -> None:
        ...
        self._process_endpoint_with_custom_schema(
            "email_event", self.client.get_email_events, email_events=email_events
        )
    ...
    def get_custom_objects(self, custom_object) -> None:
        self._process_basic_crm_object(
            custom_object, self.client.get_custom_objects, custom_object=custom_object
        )

    def _process_basic_crm_object(
        self, object_name: str, data_generator: Callable, **kwargs
    ) -> None:
        ...
        table_schema = TableSchema(
            name=object_name, primary_keys=["id"], fields=additional_property_columns
        )

        self._init_table_handler(object_name, table_schema)

        incremental_fetch_mode = (
            self._configuration.fetch_settings.fetch_mode != FetchMode.FULL_FETCH
        )

        archived = self._configuration.fetch_settings.archived

        extra_arguments = {
            "object_properties": table_schema.field_names,
            "archived": archived,
        ...
        if self._configuration.additional_properties.fetch_property_history:
            custom_props_str = getattr(
                self._configuration.additional_properties,
                f"{object_name}_property_history",
            )
    ...
    def _log_crm_object_fetching_message(self, object_name):
        ...
        if incremental_fetch_mode:
            logging_message = (
                f"{logging_message}Fetching data incrementally, from the millisecond timestamp "
    ...
    def get_all_object_columns_with_properties(
    ...
    def get_specified_object_columns_with_properties(
        self, object_name: str
    ) -> List[FieldSchema]:
        custom_props_str = getattr(
        ...
        return self._generate_field_schemas_from_properties(
            classified_object_properties
        )

    def _generate_field_schemas_from_properties(
        self, column_properties: List
    ) -> List[FieldSchema]:
        columns = []
    ...
    def fetch_and_write_to_table(
        self, object_name: str, data_generator: Callable, data_generator_kwargs
    ) -> None:
        for page in data_generator(**data_generator_kwargs):
            for item in page:
                ...
                if (
                    ...
                    and self._configuration.additional_properties.fetch_property_history
                ):
                    property_history = self._process_property_history(
                        object_name, c.get("id"), properties_with_history
                    )
    ...
    def _process_endpoint_with_custom_schema(
        self, schema_name: str, data_generator: Callable, **kwargs
    ) -> None:
    ...
    def _init_table_handler(self, handler_name, table_schema):
        if handler_name not in self._table_handler_cache:
            incremental = (
                self._configuration.destination_settings.load_mode != "full_load"
            )
            ...
            writer = ElasticDictWriter(
                table_definition.full_path, table_definition.column_names
            )
            self._table_handler_cache[handler_name] = TableHandler(
                table_definition, writer
            )

    def _add_columns_from_state_to_column_list(
        self, object_name: str, table_definition: dao.TableDefinition
    ):
        columns_in_state = self.state.get(object_name, [])
        column_names = table_definition.column_names
        state_columns_not_in_column_names = [
            col for col in columns_in_state if col not in column_names
        ]
    ...
    def _close_table_handler(self, table_handler_name: str):
        ...
        missing_columns = [
            col
            for col in final_field_names
            if col not in table_handler.table_definition.column_names
        ]
    ...
    def process_association(self, association):
        try:
            self.fetch_associations(
                from_object_type=association.from_object.value,
    ...
    def fetch_associations(
        self, from_object_type: str, to_object_type: str, id_name: str = "id"
    ):
        logging.info(
            f"Fetching v4 associations from {from_object_type} to {to_object_type}"
        )
        ...
        for page in self.client.get_associations_v4(
            object_id_generator,
            from_object_type=from_object_type,
            to_object_type=to_object_type,
        ):
            parsed_page = self._parse_association_v4(
    ...
    @staticmethod
    def _parse_association_v4(
        raw_data: List, from_object_type: str, to_object_type: str
    ):
        parsed_data = []
    ...
    def _parse_date(self, date_to_parse: str) -> int:
        if date_to_parse.lower() in {"last", "lastrun", "last run"}:
            ...
            return (
                int(
                    state.get(
                        "last_run",
                        int(dateparser.parse(DEFAULT_DATE_FROM).timestamp() * 1000),
                    )
                )
                - 3600000
            )
        try:
            ...
            raise UserException(
                f"Failed to parse date {date_to_parse}, make sure the date is either in YYYY-MM-DD "
                f"format or relative date i.e. 5 days ago, 1 month ago, yesterday, etc."
            ) from err
    ...
    def _validate_associations(self) -> None:
        fetching_endpoints = [
            endpoint_name
            for endpoint_name, fetch_endpoint in vars(
                self._configuration.endpoints
            ).items()
            if fetch_endpoint
        ]
        ...
        for endpoint in endpoints_in_associations:
            if endpoint not in fetching_endpoints:
                raise UserException(
                    f"All objects for which associations should be fetched must be present "
    ...
    @staticmethod
    def _process_property_history(
        hs_object_name, hs_object_id, properties_with_history
    ):
        history = []
        ...
        for property_history in properties_with_history:
            for history_event in properties_with_history[property_history]:
                parsed_history_event = {
                    "hs_object": hs_object_name,
    ...
    @staticmethod
    def _convert_hubspot_type_to_keboola_base_type(
        hubspot_type: str,
    ) -> SupportedDataTypes:
        type_conversions = {
            "number": SupportedDataTypes.NUMERIC,
    ...
    def _add_base_fields_to_field_schema_list(
        self, columns: List[FieldSchema]
    ) -> List[FieldSchema]:
        for base_column in [
            "archived_at",
            "archived",
    ...
    def insert_base_column(columns, column_name):
        column_exists = any(
            column_schema.name == column_name for column_schema in columns
        )
    ...
    def _fetch_object_properties(self, object_name: str) -> List[SelectElement]:
        ...
        obj_prop = self.client.get_crm_object_properties(object_name)
        return [
            SelectElement(value=prop["name"], label=f"{prop['label']} ({prop['name']})")
