...
class HubspotClient(HttpClient):
    def __init__(
        self, access_token, association_batch_size: int = DEFAULT_ASSOCIATION_BATCH_SIZE
    ):
    ...
    def get_crm_object_properties(self, object_type: str) -> List:
        try:
            return (
                self.client_v3.crm.properties.core_api.get_all(object_type=object_type)
                .to_dict()
                .get("results")
            )
    ...
    def get_contacts(
        ...
        archived: bool = False,
        since_date: str = None,
        since_property: str = "lastmodifieddate",
        properties_with_history: Optional[List] = None,
    ) -> Generator:
        return self._fetch_object_data(
    ...
    def get_companies(
        ...
        archived: bool = False,
        since_date: str = None,
        since_property: str = "hs_lastmodifieddate",
        properties_with_history: Optional[List] = None,
    ) -> Generator:
        return self._fetch_object_data(
            properties=object_properties,
    ...
    def get_owners(self, archived: bool = False) -> Generator:
        return self._paginate_v3_object(
            self.client_v3.crm.owners.owners_api,
            "owner",
            exception=owners.ApiException,
            archived=archived,
        )
    ...
    def get_deal_pipelines(self) -> List:
        try:
            return (
                self.client_v3.crm.pipelines.pipelines_api.get_all(object_type="deals")
                .to_dict()
                .get("results")
            )
    ...
    def get_ticket_pipelines(self) -> List:
        try:
            return (
                self.client_v3.crm.pipelines.pipelines_api.get_all(
                    object_type="tickets"
                )
                .to_dict()
                .get("results")
    ...
    def get_campaigns(self) -> Generator:
        for campaign_page in self._get_paged_result_pages(
            ENDPOINT_CAMPAIGNS_BY_ID, {}, "campaigns"
        ):
    ...
    def get_email_statistics(self, updated_since: Optional[int] = None) -> Generator:
        ...
        yield from self._get_paged_result_pages(
            ENDPOINT_EMAIL_STATISTICS, parameters, "objects"
        )
    ...
    def get_email_events(self, email_events: List) -> Generator:
        ...
        for event in email_events:
            yield from self._get_paged_result_pages(
                ENDPOINT_EMAIL_EVENTS, {"eventType": event}, "events"
            )
    ...
    def get_associations_v4(
        ...
        for input_chunk in self.divide_chunks(
            batch_inputs, self.association_batch_size
        ):
    ...
    def _fetch_object_data(
        ...
        **kwargs,
        ...
        if incremental:
            ...
            search_request = search_request_object(
            ...
            total = self._paginate_v3_object_search_total(
                search_api,
                endpoint_name,
            ...
            if total < HUBSPOT_API_SEARCH_LIMIT:
                return self._paginate_v3_object_search(
                    search_api,
                    endpoint_name,
                    search_request=search_request,
                    exception=exception,
    ...
    def _paginate_v3_object(
        self, api_object, endpoint_name, exception, **kwargs
    ) -> Generator:
        after = None
        while True:
    ...
    def _get_page_result(self, api_object, endpoint_name, after, exception, **kwargs):
        try:
            return api_object.get_page(
                after=after, limit=PAGE_WITH_HISTORY_MAX_SIZE, **kwargs
            )
    ...
    @staticmethod
    def _raise_exception_from_status_code(
        status_code: int, endpoint: str, reason: str = ""
    ):
        if status_code == 401:
            raise HubspotClientException(
                f"Unauthorized request, please make sure your credentials are valid. "
                f"\n {reason}"
            )
        if status_code == 403:
            raise HubspotClientException(
                f"Unauthorized request, please make sure your credentials contain the correct "
    ...
    def _parse_response_text(
        response: requests.Response, endpoint: str, parameters: Dict
    ) -> Dict:
        try:
            return response.json()
        except JSONDecodeError as e:
            raise HubspotClientException(
                f"The HS API response is invalid. endpoint: {endpoint}, "
                f"parameters: {parameters}. "
                f""
                f"Status: {response.status_code}. "
                f""
                f"Response: {response.text[:250]}... {e}"
            ) from e

    def _get_paged_result_pages_v3(
        self, endpoint: str, parameters: Dict, limit: int = PAGE_MAX_SIZE
    ):
        has_more = True
        while has_more:
            parameters["limit"] = limit
    ...
    def _paginate_v3_object_search(
        self, search_callable, endpoint_name, search_request, exception, **kwargs
    ):
        while True:
    ...
    def _paginate_v3_object_search_total(
        self, search_callable, endpoint_name, search_request, exception, **kwargs
    ):
        page = self._get_search_result(
            search_callable, endpoint_name, search_request, exception, **kwargs
    ...
    def _get_search_result(
        ...
        try:
            return search_callable(
                public_object_search_request=search_request, **kwargs
            )
        except exception as exc:
            self._raise_exception_from_status_code(exc.status, endpoint_name, exc.body)
